...
function validatePhones(element) {
  element[1] = phoneMask(element[0], element[1]);
}

function validadeMaskPhones(phones) {
  var _phones = phones;

  phones.forEach(validatePhones); 
...
export default class EditClient extends Component {
  ...
  async componentDidMount() {
    if(this.props.status!=="Edit") {
      ...
      this.setState({
        ...
        phones: validadeMaskPhones(response.data.phones),
  ...
  updateClient = async e => {
    ...
    if (response.data.cep) {
      ...
      api.put(`clients/${this.state.id}`, params)
      .then((response) => {
  ...
  render() {
    return (
      <>
        ...
        {this.state.updateDone ? <Redirect to="/adminUser"/> : (<></>)}
      </>
    );
  }
}